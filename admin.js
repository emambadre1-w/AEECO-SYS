        ...
        async function deleteUserProfile(id, name) { if (!(await confirmStyled(`${t('delete')} مستخدم "${name}"؟\nسيُحذف من جدول الملفات الشخصية ولكن سيبقى حسابه في Supabase Auth.\nلحذفه نهائياً: Authentication → Users → احذفه يدوياً.`, {type:'danger'}))) return; const { error } = await supabaseClient.from('profiles').delete().eq('id', id); if (error) { showToast('error', 'Error', error.message); return; } showToast('success', t('msg_3569a8'), name); await loadUsersModule(); }
