        ...
        function refreshAccountDropdown() {
            ...
            const inv = document.getElementById('invoiceAccount');
        ...
        function renderKitchenInvoices() {
            ...
            const statAr = { pending: t('optPendingShort'), paid: t('paid'), overdue: t('overdue'), cancelled: t('optCancelledF') };
        ...
        function renderTrialBalance() {
            ...
            var periodLabel = (fromStr||toStr) ? '<p style="font-size:12px;color:var(--text-muted);margin:0 0 10px">الفترة: '+(fromStr?formatDate(fromStr):'البداية')+' ← '+(toStr?formatDate(toStr):t('todayCardTitle'))+'</p>' : '';
        ...
        async function saveInvoice() { const id = document.getElementById('invoiceId').value || generateId(); const isNew = !document.getElementById('invoiceId').value; const invoice = {id: id, number: document.getElementById('invoiceNumber').value, client: document.getElementById('invoiceClient').value, amount: parseFloat(document.getElementById('invoiceAmount').value), status: document.getElementById('invoiceStatus').value, date: document.getElementById('invoiceDate').value || null, dueDate: document.getElementById('invoiceDueDate').value || null, notes: document.getElementById('invoiceNotes').value, accountId: (document.getElementById('invoiceAccount') ? (document.getElementById('invoiceAccount').value || null) : null), createdAt: isNew ? new Date().toISOString() : (data.invoices.find(i => i.id === id)?.createdAt || new Date().toISOString())}; const { error } = await supabaseClient.from('invoices').upsert(invoice); if (error) { showToast('error', 'Error', error.message); return; } logActivity('Invoices', isNew ? 'create' : 'update', `Invoice #${invoice.number}`); await syncInvoiceIncome(invoice); await loadInvoices(); closeModal('invoiceModal'); showToast('success', t('dataSaved'), `Invoice #${invoice.number}`); }
        async function syncInvoiceIncome(invoice) {
            try {
                // القيد ال${t('statusActive')} لهذه ال${t('docInvoiceLbl')} = ليس قيدًا عكسيًا ولم يُعكس بعد
                const existing = (data.transactions || []).find(t => t.invoiceId === invoice.id && !t.reverses_id && !t.reversed_by);
                if (invoice.status === 'paid' && !existing) {
                    await supabaseClient.from('transactions').insert({ id: generateId(), type: 'income', amount: invoice.amount, description: `تحصيل ${t('docInvoiceLbl')} #${invoice.number} - ${invoice.client}`, category: 'sales', date: invoice.date || new Date().toISOString().split('T')[0], accountId: invoice.accountId || null, invoiceId: invoice.id, createdAt: new Date().toISOString() });
                } else if (invoice.status !== 'paid' && existing) {
                    // لا نحذف القيد — ننشئ قيدًا عكسيًا حفاظًا على مسار المراجعة
                    const revId = generateId();
                    const { error: eRev } = await supabaseClient.from('transactions').insert({ id: revId, type: 'expense', amount: existing.amount, description: `${t('cancel')} تحصيل ${t('docInvoiceLbl')} #${invoice.number}` + (existing.entry_no != null ? ` (عكس القيد #${existing.entry_no})` : ''), category: existing.category || 'sales', date: new Date().toISOString().split('T')[0], accountId: existing.accountId || null, invoiceId: invoice.id, reverses_id: existing.id, createdAt: new Date().toISOString() });
                    if (!eRev) await supabaseClient.from('transactions').update({ reversed_by: revId }).eq('id', existing.id);
                }
                await loadTransactions();
            } catch (e) { console.warn('syncInvoiceIncome:', e); }
        }
        function renderInvoices() { const tbody = document.getElementById('invoicesTableBody'); if (data.invoices.length === 0) { tbody.innerHTML = `<tr><td colspan="6" style="text-align: center; padding: 40px;">${t('noData')}</td></tr>`; return; } tbody.innerHTML = data.invoices.map(inv => { const today = new Date().toISOString().split('T')[0]; const isLate = inv.status !== 'paid' && inv.status !== 'draft' && inv.dueDate && inv.dueDate < today; const effStatus = isLate ? 'overdue' : inv.status; const statusBadge = {draft: 'badge-gray', sent: 'badge-info', paid: 'badge-success', overdue: 'badge-danger'}[effStatus]; return `<tr><td style="font-family: var(--font-mono);">#${esc(inv.number)}</td><td>${esc(inv.client)}</td><td style="font-family: var(--font-mono);">${formatCurrency(inv.amount)}</td><td><span class="badge ${statusBadge}">${t(effStatus)}</span></td><td>${formatDate(inv.date)}${inv.dueDate ? `<br><span style="font-size:11px;color:${isLate ? 'var(--danger)' : 'var(--text-muted)'}">${t('dueDate')}: ${formatDate(inv.dueDate)}</span>` : ''}</td><td><button class="btn btn-sm btn-secondary" onclick="viewInvoice('${inv.id}')" title=t('view')><i class="fas fa-eye"></i></button> <button class="btn btn-sm btn-secondary" onclick="printInvoice('${inv.id}')" title=t('print')><i class="fas fa-print"></i></button> ${canEditAccounting() ? `<button class="btn btn-sm btn-secondary" onclick="editInvoice('${inv.id}')"><i class="fas fa-edit"></i></button> <button class="btn btn-sm btn-danger" onclick="deleteInvoice('${inv.id}')"><i class="fas fa-trash"></i></button>` : ''}</td></tr>`; }).join(''); }
        function editInvoice(id) { const inv = data.invoices.find(i => i.id === id); if (inv) { document.getElementById('invoiceId').value = inv.id; document.getElementById('invoiceNumber').value = inv.number; document.getElementById('invoiceClient').value = inv.client; document.getElementById('invoiceAmount').value = inv.amount; document.getElementById('invoiceStatus').value = inv.status; document.getElementById('invoiceDate').value = inv.date; document.getElementById('invoiceDueDate').value = inv.dueDate || ''; document.getElementById('invoiceNotes').value = inv.notes; refreshAccountDropdown(); var _ia=document.getElementById('invoiceAccount'); if(_ia) _ia.value = inv.accountId || ''; openModal('invoiceModal'); } }
        ...
        function viewInvoice(id){ var r=(data.invoices||[]).find(function(x){return x.id===id;}); if(!r) return; showDetailsModal('تفاصيل الفاتورة', [[t('invoiceNumber'),esc(r.number||'-')],[t('client'),esc(r.client||'-')],[t('amount'),r.amount!=null?formatCurrency(r.amount):'-'],[t('status'),esc(r.status||'-')],[t('createdAt'),r.date?formatDate(r.date):'-'],[t('dueDate'),r.dueDate?formatDate(r.dueDate):'-'],[t('notes'),esc(r.notes||'-')]]); }
        function viewAccount(id){ var r=(data.accounts||[]).find(function(x){return x.id===id;}); if(!r) return; showDetailsModal('تفاصيل الحساب', [[t('accountNumber'),esc(r.number||'-')],[t('name'),esc(r.name||'-')],[t('type'),esc(r.type||'-')],[t('balance'),r.balance!=null?formatCurrency(r.balance):'-']]); }
        function viewTransaction(id){
            var r=(data.transactions||[]).find(function(x){return x.id===id;});
            if(!r) return;
            var rows = [
                [t('entryNo'), r.entry_no != null ? String(r.entry_no) : '—'],
                [t('type'), esc(r.type||'-')],
                [t('category'), esc(r.category||'-')],
                [t('description'), esc(r.description||'-')],
                [t('amount'), r.amount!=null?formatCurrency(r.amount):'-'],
                [t('createdAt'), r.date?formatDate(r.date):'-']
            ];
            if (r.reversed_by) {
                var rev = (data.transactions||[]).find(function(x){ return x.id === r.reversed_by; });
                rows.push([t('status'), 'معكوس بالقيد رقم ' + (rev && rev.entry_no != null ? rev.entry_no : '—')]);
            } else if (r.reverses_id) {
                var orig = (data.transactions||[]).find(function(x){ return x.id === r.reverses_id; });
                rows.push([t('status'), 'قيد عكسي للقيد رقم ' + (orig && orig.entry_no != null ? orig.entry_no : '—')]);
            }
