        ...
        async function syncInvoiceIncome(invoice) {
            try {
                // القيد النشط لهذه الفاتورة = ليس قيدًا عكسيًا ولم يُعكس بعد
                ...
                if (invoice.status === 'paid' && !existing) {
                    await supabaseClient.from('transactions').insert({ id: generateId(), type: 'income', amount: invoice.amount, description: `تحصيل فاتورة #${invoice.number} - ${invoice.client}`, category: 'sales', date: invoice.date || new Date().toISOString().split('T')[0], accountId: invoice.accountId || null, invoiceId: invoice.id, createdAt: new Date().toISOString() });
                } else if (invoice.status !== 'paid' && existing) {
                    // لا نحذف القيد — ننشئ قيدًا عكسيًا حفاظًا على مسار المراجعة
                    const revId = generateId();
                    const { error: eRev } = await supabaseClient.from('transactions').insert({ id: revId, type: 'expense', amount: existing.amount, description: `إلغاء تحصيل فاتورة #${invoice.number}` + (existing.entry_no != null ? ` (عكس القيد #${existing.entry_no})` : ''), category: existing.category || 'sales', date: new Date().toISOString().split('T')[0], accountId: existing.accountId || null, invoiceId: invoice.id, reverses_id: existing.id, createdAt: new Date().toISOString() });
