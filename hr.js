        ...
        function toggleAmountField() { const type = document.getElementById('reqType').value; const showAmount = type === 'expense' || type === 'purchase' || type === 'financial_certification'; document.getElementById('reqAmountGroup').classList.toggle('app-hidden', !showAmount); const lg = document.getElementById('reqLeaveGroup'); if(lg) lg.classList.toggle('app-hidden', type !== 'leave'); const fc = document.getElementById('reqFinCertGroup'); if(fc) fc.classList.toggle('app-hidden', type !== 'financial_certification'); const dcb = document.getElementById('reqDirectCertBtn'); if(dcb) dcb.classList.toggle('app-hidden', !(type === 'financial_certification' && currentUser && currentUser.role === 'admin')); renderStagePreview(type); }
        async function submitAndCertifyNow(){
            if (!currentUser || currentUser.role !== 'admin') { showToast('error', t('accessDenied'), t('accessDeniedMsg')); return; }
        ...
        async function loadMyHRInfo() {
            ...
            const [payRes, lvRes, asRes] = await Promise.all([
                ...
                supabaseClient.from('fixed_assets').select('*').eq('employee_id', empId).order('name')
            ]);
            myHRData.payroll = payRes.data || [];
            myHRData.leaves = lvRes.data || [];
            myHRData.assets = (asRes && asRes.data) || [];
            renderMyHRPage();
        }
        function renderMyHRPage() {
            ...
            const asCard = document.getElementById('myHrAssetsCard');
            const asBody = document.getElementById('myHrAssetsBody');
            const myAssets = (myHRData.assets || []).filter(function(a){ return (a.status||'active') !== 'sold' && (a.status||'active') !== 'written_off'; });
            if (asCard) asCard.classList.toggle('app-hidden', myAssets.length === 0);
            if (asBody && myAssets.length) {
                asBody.innerHTML = myAssets.map(function(a){
                    var catLbl = (typeof assetCategoryLabel === 'function') ? assetCategoryLabel(a.category) : esc(a.category || '-');
                    var stLbl  = (typeof assetStatusBadge  === 'function') ? assetStatusBadge(a.status || 'active') : esc(a.status || '-');
                    return '<tr><td style="font-family:var(--font-mono)">' + esc(a.code || '-') + '</td><td>' + esc(a.name || '-') + '</td><td>' + catLbl + '</td><td>' + esc(a.location || '-') + '</td><td>' + stLbl + '</td></tr>';
                }).join('');
            }
