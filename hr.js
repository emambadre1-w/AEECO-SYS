        async function loadEmployees() { try { const { data: rows, error } = await supabaseClient.from('employees').select('*').order('createdAt', { ascending: false }); if (error) console.warn('employees:', error.message); else data.employees = rows || []; } catch (e) { console.warn('employees:', e); } renderEmployees(); const el = document.getElementById('statEmployees'); if (el) el.textContent = data.employees.length; renderLeaveBalances(); }
        async function loadAttendance() { try { const { data: rows, error } = await supabaseClient.from('attendance').select('*').order('date', { ascending: false }); if (error) console.warn('attendance:', error.message); else data.attendance = rows || []; } catch (e) { console.warn('attendance:', e); } renderAttendance(); }
        async function loadLeaves() { try { const { data: rows, error } = await supabaseClient.from('leaves').select('*').order('createdAt', { ascending: false }); if (error) console.warn('leaves:', error.message); else data.leaves = rows || []; } catch (e) { console.warn('leaves:', e); } renderLeaves(); renderLeaveBalances(); }
        ...
        async function openAddEmployeeModal() { document.getElementById('employeeForm').reset(); document.getElementById('employeeId').value = ''; await populateEmployeeDropdowns(null); toggleContractEndField(); document.getElementById('empDocLabel').value = ''; const df = document.getElementById('empDocFile'); if (df) df.value = ''; document.getElementById('empDocUploadStatus').textContent = ''; document.getElementById('empDocumentsList').innerHTML = '<div style="font-size:12px;color:var(--text-muted);">احفظ الموظف أولاً قبل إضافة مستندات</div>'; openModal('employeeModal'); }
        ...
        async function populateEmployeeDropdowns(currentEmpId) {
        ...
        async function editEmployee(id) { const emp = data.employees.find(e => e.id === id); if (emp) { document.getElementById('employeeId').value = emp.id; document.getElementById('empId').value = emp.empId; document.getElementById('empName').value = emp.name; document.getElementById('empPosition').value = emp.position; document.getElementById('empDepartment').value = emp.department; document.getElementById('empPhone').value = emp.phone; document.getElementById('empEmail').value = emp.email; document.getElementById('empHireDate').value = emp.hireDate; document.getElementById('empSalary').value = emp.salary; document.getElementById('empAnnualLeave').value = (emp.annualLeave != null ? emp.annualLeave : 21); document.getElementById('empStatus').value = emp.status || 'active'; document.getElementById('empNationalId').value = emp.nationalId || ''; document.getElementById('empBankAccount').value = emp.bankAccount || ''; document.getElementById('empEmergencyName').value = emp.emergencyContactName || ''; document.getElementById('empEmergencyPhone').value = emp.emergencyContactPhone || ''; await populateEmployeeDropdowns(emp.id); document.getElementById('empManagerId').value = emp.managerId || ''; document.getElementById('empProfileId').value = emp.profileId || ''; document.getElementById('empContractType').value = emp.contractType || 'permanent'; document.getElementById('empContractEndDate').value = emp.contractEndDate || ''; toggleContractEndField(); document.getElementById('empDocLabel').value = ''; const df = document.getElementById('empDocFile'); if (df) df.value = ''; document.getElementById('empDocUploadStatus').textContent = ''; loadEmployeeDocuments(emp.id); openModal('employeeModal'); } }
