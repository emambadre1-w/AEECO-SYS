        ...
        async function completeMilestone(id) { const{error}=await supabaseClient.from('pm_milestones').update({completed:true,completed_at:new Date().toISOString()}).eq('id',id); if(error){showToast('error','Error',error.message);return;} showToast('success',t('msg_520799'),''); await loadPMModule(); }
        async function editPMProgress(id,current) { const val=prompt('نسبة الإنجاز الجديدة (0-100):',current); if(val===null)return; const n=Math.max(0,Math.min(100,parseInt(val)||0)); const{error}=await supabaseClient.from('pm_projects').update({progress:n}).eq('id',id); if(error){showToast('error','Error',error.message);return;} showToast('success',t('msg_815dc4'),n+'%'); await loadPMModule(); }
        ...
        async function deletePMProject(id){
            if(!(await confirmStyled(t('msg_e24dd7'), {type:'danger'})))return;
            try{
            ...
            showToast('success',t('msg_dcfe68'),'');
        ...
        function printProject(id){
            ...
            if(!win){ showToast('error',t('msg_8758f2'),t('msg_b61a8e')); return; }
