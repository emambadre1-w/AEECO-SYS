        async function onAuthSuccess() {
            ...
            checkBackupReminder(); checkMeetingReminder(); navigateTo(_target); applyEditVisibility();
        }
        ...
        async function renderRoleDashboard() {
            ...
            if (role === 'employee') {
                ...
                html = `<div class="role-stats">${roleStatCard(my.length,'إجمالي طلباتي','fa-file-alt','primary')}${roleStatCard(my.filter(r=>r.status==='pending').length,t('statusPending'),'fa-clock','warning')}${roleStatCard(my.filter(r=>r.status==='approved').length,'معتمدة','fa-check-circle','success')}${roleStatCard(my.filter(r=>r.status==='rejected').length,'مرفوضة','fa-times-circle','danger')}</div><div class="card"><div class="card-header"><h3 class="card-title">آخر طلباتي</h3><button class="btn btn-sm btn-secondary" onclick="navigateTo('requests')">عرض الكل</button></div><div class="card-body"><div class="table-container"><table><thead><tr><th>العنوان</th><th>النوع</th><th>الحالة</th><th>التاريخ</th></tr></thead><tbody>${my.slice(0,5).map(r=>`<tr><td>${r.title}</td><td><span class="badge badge-info">${t('reqType'+r.type.charAt(0).toUpperCase()+r.type.slice(1))}</span></td><td><span class="badge ${{pending:'badge-warning',approved:'badge-success',rejected:'badge-danger'}[r.status]}">${t('status'+r.status.charAt(0).toUpperCase()+r.status.slice(1))}</span></td><td>${formatDateTime(r.created_at)}</td></tr>`).join('')||`<tr><td colspan="4" style="text-align:center;padding:24px;">${t('noRequests')}</td></tr>`}</tbody></table></div></div></div>`;
            } else if (role === 'hr') {
                const pendingHR = allRequestsCache.filter(r => r.status==='pending' && r.current_stage==='hr');
                html = `<div class="role-stats">${roleStatCard(pendingHR.length,t('pendingMyApproval'),'fa-inbox','warning')}${roleStatCard((data.employees||[]).length,t('totalEmployees'),'fa-users','primary')}${roleStatCard(allRequestsCache.filter(r=>r.type==='leave').length,t('leaveRequests'),'fa-calendar-alt','info')}${roleStatCard((data.tickets||[]).filter(tk=>tk.status==='open').length,'تذاكر IT مفتوحة','fa-ticket-alt','danger')}</div>${rolePendingTable(pendingHR,'طلبات بانتظار موافقة HR')}`;
            } else if (role === 'accountant') {
                const pendingAcc = allRequestsCache.filter(r => r.status==='pending' && r.current_stage==='accountant');
                html = `<div class="role-stats">${roleStatCard(pendingAcc.length,t('pendingMyApproval'),'fa-inbox','warning')}${roleStatCard(formatCurrency(income),t('totalIncome'),'fa-arrow-down','success')}${roleStatCard(formatCurrency(expenses),t('totalExpenses'),'fa-arrow-up','danger')}${roleStatCard(formatCurrency(income-expenses),t('netBalance'),'fa-wallet','primary')}</div>${rolePendingTable(pendingAcc,'طلبات بانتظار موافقة المحاسب')}`;
            } else if (role === 'gm') {
                const pendingGM = allRequestsCache.filter(r => r.status==='pending' && r.current_stage==='gm');
                html = `<div class="role-stats">${roleStatCard(pendingGM.length,'بانتظار قراري','fa-inbox','warning')}${roleStatCard(allRequestsCache.filter(r=>r.status==='pending').length,'إجمالي المعلقة','fa-clock','danger')}${roleStatCard(formatCurrency(income),t('totalIncome'),'fa-dollar-sign','success')}${roleStatCard((data.employees||[]).length,t('totalEmployees'),'fa-users','primary')}</div>${rolePendingTable(pendingGM,'طلبات بانتظار موافقة المدير العام')}`;
            } else if (role === 'pm') {
                ...
                html = `<div class="role-stats">${roleStatCard(lowStock.length,'منتجات قاربت تنفد','fa-triangle-exclamation','danger')}${roleStatCard((data.products||[]).length,t('totalProducts'),'fa-boxes-stacked','primary')}${roleStatCard((data.stockMovements||[]).length,t('stockMovements'),'fa-exchange-alt','success')}${roleStatCard(openTickets,t('openTickets'),'fa-ticket-alt','warning')}</div><div class="card"><div class="card-header"><h3 class="card-title">منتجات قاربت تنفد</h3><button class="btn btn-sm btn-secondary" onclick="navigateTo('inventory')">عرض الكل</button></div><div class="card-body"><div class="table-container"><table><thead><tr><th>المنتج</th><th>الكمية</th><th>حد الطلب</th></tr></thead><tbody>${lowStock.slice(0,6).map(it=>`<tr><td>${esc(it.name)}</td><td><span class="badge badge-danger">${it.quantity||0}</span></td><td>${it.reorder_level}</td></tr>`).join('')||`<tr><td colspan="3" style="text-align:center;padding:24px;">لا توجد منتجات قاربت تنفد ✓</td></tr>`}</tbody></table></div></div></div>`;
            }
        ...
        async function saveKitchenMovement() {
            ...
            logActivity('Kitchen', 'create', (it ? it.name : '') + ' — ' + (type === 'in' ? t('optStockIn') : t('optStockOut')));
            if (it) {
        ...
        function renderKitchenExpenses() {
            ...
            if (totalEl) totalEl.textContent = `${t('grossSalary')}: ${total.toLocaleString()} ${cur}`;
